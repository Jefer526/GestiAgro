...
const Historial_trabajo_agro = () => {
  ...
  // Estado menú perfil flotante
  const [mostrarTarjeta, setMostrarTarjeta] = useState(false);

  // Cierra el menú perfil al hacer click fuera
      ...
      <div className="flex-1 p-10 overflow-auto">
        ...
        <div className="bg-white border border-gray-300 p-6 rounded-xl mb-6 max-w-4xl">
          ...
          <div className="grid grid-cols-2 gap-x-12 gap-y-2 text-lg">
            ...
            <p>
              <strong>Máquina:</strong> {infoGeneral.maquina}
            </p>
            <p>
              <strong>Estado:</strong> {infoGeneral.estado}
            </p>
            <p>
              <strong>Referencia:</strong> {infoGeneral.referencia}
            </p>
          </div>
        </div>
            ...
            >
              ...
              {Object.entries(
                historial.reduce((acc, { fecha }) => {
                  ...
                  const monthName = new Date(fecha).toLocaleString("default", {
                    month: "long",
                  });
                  acc[year] = acc[year] || {};
                  ...
                  {Object.entries(months).map(([month, days]) => (
                    <div key={month} className="ml-4">
                      ...
                      {[...days].map((day) => {
                        ...
                        const fullDate = `${year}-${String(monthNum).padStart(
                          2,
                          "0"
                        )}-${day}`;
                        return (
                          <label key={day} className="ml-6 flex items-center gap-2">
                            <input
                              type="checkbox"
                              checked={(valoresSeleccionados["fecha"] || []).includes(
                                fullDate
                              )}
