...
const Editar_lote = () => {
  ...
  // Estado de alerta de confirmación
  const [alertaVisible, setAlertaVisible] = useState(false);

  // Estado y referencia para el menú de perfil
  const [mostrarTarjeta, setMostrarTarjeta] = useState(false);
  const tarjetaRef = useRef(null);

  // Datos de usuario para inicial del perfil
  const nombreUsuario = "Juan Pérez"; // Cambiar por usuario real
  const letraInicial = (nombreUsuario?.trim()?.[0] || "U").toUpperCase();

  // Cierra tarjeta de perfil al hacer click fuera
  useEffect(() => {
      ...
      <div className="flex-1 p-10 overflow-y-auto">
        ...
        <div className="bg-white shadow-md border border-green-300 p-8 rounded-xl w-full max-w-3xl mx-auto space-y-6 min-h-[36rem] flex flex-col justify-between">
          ...
          <div>
            ...
            <input name="finca" value={formData.finca} onChange={handleChange} placeholder="Nombre finca" className="w-full border p-3 rounded text-base" />
          </div>

          <div>
            <label className="block mb-1 font-semibold text-black">Número de lote</label>
            <input name="lote" value={formData.lote} onChange={handleChange} placeholder="Número" className="w-full border p-3 rounded text-base" />
          </div>

          <div>
            <label className="block mb-1 font-semibold text-black">Coordenadas</label>
            <input name="coordenadas" value={formData.coordenadas} onChange={handleChange} placeholder="Lat,Lng" className="w-full border p-3 rounded text-base" />
          </div>

          <div>
            <label className="block mb-1 font-semibold text-black">Área lote</label>
            <input name="area" value={formData.area} onChange={handleChange} placeholder="Área" className="w-full border p-3 rounded text-base" />
          </div>

          <div>
            <label className="block mb-1 font-semibold text-black">Cultivo</label>
            <input name="cultivo" value={formData.cultivo} onChange={handleChange} placeholder="Tipo cultivo" className="w-full border p-3 rounded text-base" />
          </div>
          ...
          <div className="flex justify-center space-x-6 mt-6">
            ...
            <button onClick={() => navigate("/crearlote")} className="bg-gray-300 text-black px-8 py-2 rounded hover:bg-gray-400">
              Cancelar
            </button>
          </div>
        </div>
