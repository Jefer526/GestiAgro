...
import {
  IconHome, IconClipboardList, IconChartBar, IconBox, IconCloudRain, IconTractor,
  IconUsersGroup, IconPlant, IconFrame, IconSettings, IconTool, IconLogout,
  IconChevronLeft, IconCheck, IconDotsVertical, IconAlertTriangle, IconCircleCheck,
  IconFilter, IconSortAscending2, IconSortDescending2, IconPlant2, IconBook
...
const Registrar_novedad_agro = () => {
  ...
  // Cerrar tarjeta perfil
  useEffect(() => {
    const manejarClickFuera = (e) => {
      if (tarjetaRef.current && !tarjetaRef.current.contains(e.target)) setMostrarTarjeta(false);
    };
    ...
    <div className="flex">
      ...
      <div className="flex-1 p-10 overflow-auto relative bg-gray-50">
        ...
        <div className="overflow-x-auto rounded-lg shadow-lg relative bg-white">
                  ...
                  <tr key={m.id} className="hover:bg-gray-100">
                    ...
                    <td className="p-4 border">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button onClick={(e) => openMenu(index, e)} className={`inline-flex items-center justify-center w-9 h-9 rounded-lg border shadow-sm transition ${isAveriado ? "border-red-200 bg-red-50 text-red-600 hover:bg-red-100" : "border-gray-200 bg-gray-50 text-gray-600 hover:bg-gray-100"}`}>
          ...
          {/* Filtros */}
          {filtroActivo && (
            <div ref={filtroRef} className="fixed bg-white text-black shadow-md border rounded z-[1100] p-3 w-60 text-left text-sm" style={{ top: filtroPosicion.top, left: filtroPosicion.left }}>
              <div className="font-semibold mb-2">
                Filtrar por {filtroActivo.charAt(0).toUpperCase() + filtroActivo.slice(1)}
              </div>
              <button onClick={() => ordenar(filtroActivo, "asc")} className="text-green-700 flex items-center gap-1 mb-1 capitalize"><IconSortAscending2 className="w-4 h-4" /> Ordenar A → Z</button>
              <button onClick={() => ordenar(filtroActivo, "desc")} className="text-green-700 flex items-center gap-1 mb-2 capitalize"><IconSortDescending2 className="w-4 h-4" /> Ordenar Z → A</button>
        ...
        {menuIndex !== null && (
          <MenuOverlay>
            <div ref={menuRef} className="fixed w-72 bg-white border border-gray-100 rounded-2xl shadow-2xl p-2 z-[1200]" style={{ top: menuPos.top, left: menuPos.left }}>
              ...
              {maquinas[menuIndex]?.estado !== "Averiado" && (
                <button onClick={() => { handleEstadoChange(menuIndex, true); closeMenu(); }} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl transition text-left hover:bg-gray-100">
              ...
              {maquinas[menuIndex]?.estado === "Averiado" && (
                <button onClick={() => { handleEstadoChange(menuIndex, false); closeMenu(); }} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl transition text-left hover:bg-gray-100">
                  <div className="shrink-0 inline-flex items-center justify-center w-8 h-8 rounded-lg border">
                    <IconCircleCheck className="w-5 h-5" />
                  </div>
                  <div>
        ...
        <div className="flex justify-center mt-6">
          <button onClick={handleGuardar} className="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700 text-lg font-semibold">
            Guardar Cambios
          </button>
        </div>
      </div>
